...
function _resetGameLoopClock() { _loopLast = _perfNow(); _tickDebt = 0; }
...
//    錨點法對「節流」與「凍結」一視同仁。真正關閉網頁時這筆帳隨頁面記憶體消失，改由 js/27 離線結算接手（不重複）。
...
if (typeof document !== 'undefined' && document.addEventListener) {
    document.addEventListener('visibilitychange', function () {
        ...
        _loopLast = _perfNow();   // 回前景：時鐘重設（背景段不經 elapsed 路徑、由下行整段入帳）
        if (_anchor > 0 && typeof state !== 'undefined' && state.running && typeof player !== 'undefined' && player && player.cls && !player.dead) {
            _tickDebt += Math.max(0, _perfNow() - _anchor);
        }
    });
}
if (typeof window !== 'undefined' && window.addEventListener) {
    // bfcache 還原＝視同「關閉後重開」：時間帳歸零（js/27 於 pageshow 走離線結算補償這段·雙軌不得重複入帳）
    window.addEventListener('pageshow', function () { _ffHiddenAt = 0; _resetGameLoopClock(); });
}
