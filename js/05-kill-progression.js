...
function killMob(idx) {
    ...
    player.exp += Math.floor(mob.exp * getExpGainMult(player.lv) * (1 + dollFieldVal('expBonus') / 100));   // ⚠️v3.0.82 經典模式經驗×0.5 已移除（一般＝經典）；🪆 魔法娃娃 expBonus%
    checkLvUp();
    ...
    if (player.allies && player.allies.length && mob.exp) {
        player.allies.forEach(a => {
            ...
            let _gain = Math.floor(mob.exp * getExpGainMult(a.lv || 1) * MERC_EXP_SHARE);   // ⚠️v3.0.82 經典×0.5 已移除
            if (_gain <= 0) return;
            a.exp = (a.exp || 0) + _gain;
