...
function _petOutStateKey(p) { return (p && p.outSlot != null) ? String(p.outSlot) : ''; }
...
function petRoster() {
    ...
    if (_petRosterKey !== key) {
        ...
        _petRoster.forEach(p => {
            ...
            // 舊 v2 只有全域 `out`，無法可靠判斷屬於哪個角色；升級時一律安全返回保管，避免雙視窗各自認領。
            if (p.outSlot == null && p.out) { p.outSlot = null; p.outV = _petNowStamp(); _petRosterDirty = true; }
...
function _petMergeFromBucket(cur, key) {
    ...
    _petRoster.forEach(p => {
        ...
        if (_petOutStateKey(p) && _petOutStateKey(p) !== String(currentSlot) && _petOutStateKey(f) === _petOutStateKey(p) && _fOutV === (Number(p.outV) || 0)) {
...
function petReleaseSlotAssignments(slot) {
    let owner = String(slot), changed = false;
    for (let key of [PET_ROSTER_KEY, PET_ROSTER_KEY + '_classic']) {
        ...
        let dirty = false;
        list.forEach(p => { if (p && p.outSlot != null && String(p.outSlot) === owner) { p.outSlot = null; p.outV = _petNowStamp(); dirty = true; } });
        if (!dirty) continue;
...
function _petOwnedByOther(p) { return !!(p && p.outSlot != null && String(p.outSlot) !== String(currentSlot)); }
